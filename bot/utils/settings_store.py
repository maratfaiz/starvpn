...
Card и Crypto по умолчанию ВЫКЛЮЧЕНЫ (продуктовое решение — временно
принимаем оплату только через ЮMoney). Stars тоже выключены той же
командой (см. ADR-010) — раньше были "не тумблятся, всегда доступны",
теперь это тоже управляемый тумблер. Код и вебхуки/обработчики намеренно
не удалены — админ может снова включить любой способ из панели без
деплоя, когда потребуется.
"""
