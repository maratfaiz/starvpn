...
После первого разворачивания сервер обновляется сам: `infra/auto-deploy.sh`
раз в несколько минут проверяет `origin/original` и, если появились новые
коммиты, подтягивает их и пересобирает контейнеры — `git push` доезжает
до продакшена без ручного захода на сервер. Установка (один раз) и
детали — в [`AGENTS/roles/devops.md`](./AGENTS/roles/devops.md).
