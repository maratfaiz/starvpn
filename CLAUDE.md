...
3. **Validate all webhook signatures**: Robokassa → MD5 HMAC; CryptoPay → HMAC-SHA256. Reject unsigned requests with HTTP 403.
...
- **Referral bonus (2026-09-05, ADR-018 — reconciles two parallel redesigns, ADR-017 and a same-day sibling branch)**: the referee gets `REFEREE_BONUS_DAYS = 2` immediately on their first paid purchase via the link (any method — Stars/card/crypto, not gifts). The referrer gets `REFERRAL_DAYS_PER_REFERRAL = 15` days per paying referral, but only after a **30-day vesting hold** (`REFERRAL_VESTING_DAYS`) — the referred user must still be an active, non-banned subscriber 30 days after their *first* payment before the referrer is credited; this is the standard NordVPN/ExpressVPN/Surfshark/Proton anti-fraud pattern (payment-gating alone isn't enough, a fast pay-then-refund/cancel cycle must not pay out). Credited by `credit_vested_referrals()` (`bot/handlers/payment.py`), which runs hourly from `bot/tasks/scheduler.py::scheduler_loop` — **not** at payment time. Capped at `REFERRAL_MONTHLY_CAP_DAYS = 90` days per rolling 30-day window per referrer, tracked via a `referral_credits` ledger table (per-grant audit trail; also what the cap sums over). Achievement badges (`REFERRAL_ACHIEVEMENTS`, 1/5/10/25 paying referrals) are status-only — no bonus days attached (an earlier version stacked bonus days on top of a recurring pack and could reach 585 days on 25 referrals). Fixes two real bugs that predated both redesigns: `referral_count` used to be double-incremented (once at `/start` before any payment, again at first payment) — it now increments exactly once, inside `credit_vested_referrals()`; and the referral bonus used to fire only for Telegram Stars purchases — `_mark_first_payment()` is now called from the Stars, card, and crypto payment paths alike. See `AGENTS/architecture/referral-system.md` for the full mechanic and `AGENTS/decisions/ADR.md` (ADR-018) for how the two parallel branches were reconciled.
