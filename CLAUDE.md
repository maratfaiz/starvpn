...
- **Support tickets (2026-09-03, redesigned)**: all three "support" entry points (bot's "🎧 Поддержка" button, Mini App's "Написать в поддержку" tile, `GET /api/me`'s `support_url`) now point to `{SITE_URL}/support` instead of a raw `@hashprojects` DM — the ticket system used to only be reachable from the website. Tickets are now a real thread (`support_ticket_messages`, one row per message) instead of a single `admin_reply` string that could hold exactly one reply ever; also gained `priority` (low/normal/urgent) and `assigned_admin_id` on `SupportTicket`. When an admin replies and the ticket owner has a real (positive) `telegram_id`, the bot message carries a "✍️ Ответить" inline button that arms `SupportForm.waiting_reply` (`bot/handlers/support.py`) so the user's next message appends to the same thread — email-contact and web-account tickets stay one-way (no inbound mail parsing exists). `settings.support_username`'s raw DM is kept only as a fallback mention in 2-3 error-message contexts (dashboard link-unavailable, ban notice), not as a support channel. See `AGENTS/architecture/support-system.md` for the full design, what changed in the admin panel (priority/assignment filters, a `goto()` search bug fixed along the way), and what's not implemented (FAQ deflection, CSAT, SLA escalation, a real admin picker for assignment).
