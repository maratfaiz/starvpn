...
- **Payment methods (2026-08-30, ADR-013)**: card (Robokassa) and crypto (CryptoPay) are ON; ЮMoney and Telegram Stars are OFF — `bot/utils/settings_store.py` `_DEFAULT_ENABLED`. This has flipped 4 times in one day (ADR-005 → 010 → 012 → 013); a code default only matters for a fresh deploy with no `app_settings` row — an already-toggled prod row wins over it. Don't remove `card_payment.py`/`crypto_payment.py`/`yoomoney_payment.py`/the Stars invoice code because they "look unused" — flip them back on from `/admin` → Настройки, no deploy needed. Because "zero providers enabled" turned out to be a real state at one point, `bot/handlers/profile.py::pay_choice_text()` and `landing/get-vpn.html`'s payment step both explicitly handle it. Crypto payment (via @CryptoBot) always requires Telegram regardless of how the account was created — that's a CryptoPay constraint, not ours; card is the only truly Telegram-independent method. Site-based invoice creation (`/api/invoice/card`, `/api/invoice/crypto`, mirrored by `/api/card/plans`/`/api/crypto/plans`) only supports the 3 fixed plan tiers — no custom-day pricing (that only exists in the guest-checkout path, see below).
