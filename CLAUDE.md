...
- **Referral bonus (2026-08-31, redesigned; cap changed same day)**: +15 days to the referrer per paying referral (flat, not "every 2" batching), credited only after a **30-day vesting hold** — the referred user must still be an active, non-banned subscriber 30 days after their *first* payment (any method: Stars/card/crypto, not gifts) before the referrer gets credited. Capped at **90 days per rolling 30-day window** per referrer (`REFERRAL_MONTHLY_CAP_DAYS` — changed same day from an initial 365-days/365-day-window cap, per explicit product decision to simplify the Mini App's Referrals screen copy to "90 days / 6 friends a month"; the window is rolling, not calendar-month, so it doesn't reset on the 1st). Achievement badges (1/5/10/25 paying referrals) are status-only — no bonus days attached (previously stacked on top of the base bonus and could reach 585 days on 25 referrals); still shown in the bot's "👥 Партнёрка" message and on the wiki page, but deliberately dropped from the Mini App's Referrals screen (2026-08-31) to keep that screen as simple as possible — this is an intentional cross-surface inconsistency, not a bug. Tracked via `User.extra_days_granted` (running total) + the `referral_credits` table (per-grant audit trail, also the basis for the monthly cap). See `AGENTS/architecture/referral-system.md` for the full mechanic, the two bugs this fixed (double-counted `referral_count`, Stars-only crediting), and what's still not implemented (refund clawback — no refund flow exists anywhere yet; device/IP fraud fingerprinting).
- **Support tickets (2026-09-03, redesigned)**: every "support" surface (bot's "🎧 Поддержка" button, Mini App's "Написать в поддержку" tile, `GET /api/me`'s `support_url`, the ban notice, the Marzban-link-unavailable error, the wiki-page footer) now points to `{SITE_URL}/support` instead of a raw `@hashprojects` DM — `settings.support_username`/`SUPPORT_USERNAME` was removed from `bot/config.py` entirely, there is no code path left that hands out a personal DM as a support channel (a user who already knows that Telegram handle can still message it directly — that's a platform limitation, not something code controls). Tickets are now a real thread (`support_ticket_messages`, one row per message) instead of a single `admin_reply` string that could hold exactly one reply ever; also gained `priority` (low/normal/urgent) and `assigned_admin_id` on `SupportTicket`. When an admin replies and the ticket owner has a real (positive) `telegram_id`, the bot message carries a "✍️ Ответить" inline button that arms `SupportForm.waiting_reply` (`bot/handlers/support.py`) so the user's next message appends to the same thread — email-contact and web-account tickets stay one-way (no inbound mail parsing exists). The admin panel's "Обращения" section is a plain table (matching Users/Payments/Referrals); the detail panel does NOT render the thread as a message log/chat (that was tried and explicitly rejected as still looking like a chat) — it shows one `.tkt-msg` card with only the user's latest message (original text, or whatever they most recently sent back via "✍️ Ответить") plus a `.tkt-acc` card naming the linked account (if any) with an "Открыть аккаунт" button reusing `openUserDetail()`. The full thread is still stored (`support_ticket_messages`) and used server-side to find the latest user message, just never rendered as a list. `GET /web/tickets` rows carry `account_linked: bool` (small "Аккаунт" badge next to the user label) — this only means the ticket's `user_id` was set because the submitter had an active session/initData when they filled the form, NOT an admin login-as-user feature. The public form (`landing/support.html`) has no alternate contact channels anymore (the "Другие способы" sidebar card — Telegram chat link + troubleshooting link — was removed, everything funnels through the ticket form). "Как с вами связаться" is a 3-way tab choice — **Аккаунт** / **Email** / **Telegram** — not a free-text field; logging in is still optional (Email/Telegram work for a guest with zero auth calls). Picking the "Аккаунт" tab without a session saves the in-progress draft (topic/platform/message) to `localStorage['star_vpn_pending_support_draft']` and sends the visitor to `/login?next=/support`; `/login`'s two post-auth redirects and both Telegram-OAuth links now honor `?next=` against a hardcoded allowlist (`/account`, `/support` — see `_OAUTH_ALLOWED_NEXT` in `bot/api.py`), and the OAuth PKCE cookie carries the next-path as a third dot-separated field so `GET /api/telegram-oauth/callback` knows where to land. Back on `/support`, the draft is restored and cleared, and the account tab auto-activates once a session exists. `POST /api/support`'s `anonymous` flag is now `true` whenever the contact mode isn't "account" — picking Email/Telegram anonymizes the ticket even with a live session, no separate opt-out toggle. The account's own view lives inside `/account`'s **Профиль** tab (not a separate top-level tab — merged there on request, "should be where the account is"), as a "Мои обращения" card right under the account-info card (`GET /api/support/tickets` list + `GET /api/support/tickets/{id}` detail, ownership-checked via `t.user_id != tg_id` → 404) — rendered as an accordion (`.tkt-list`/`.tkt-item`, colored left border by status, click expands the thread in place), not a table. The `S-NNNNNN` public ID is admin-only now — the account's own list identifies a ticket by topic + creation date instead (`t.id` still exists, just never rendered there). If the ticket isn't closed, the expanded view has a reply box — `POST /api/support/tickets/{id}/reply` (ownership-checked the same way) lets the account holder append a message and reopens the ticket (`status="open"`), the same effect as the bot's "✍️ Ответить" flow, just a second path to it; a closed ticket shows a "оформите новое" note instead, and the server also rejects (400) a reply attempt on an already-closed ticket. The admin reply hint appends "Также появится в «Мои обращения» личного кабинета." whenever the ticket has a linked account — the reply was always saved to the thread regardless of channel, this just makes that visible in the admin UI — unlike the admin panel, showing the full back-and-forth on the account's own page is fine (it's the user's own thread, not admin UI), rendered as plain stacked cards, no bubbles. `landing/support.html` carries the same full `<nav>` (nav-links, account-aware pill/dropdown, mobile hamburger) as every other subpage (`tariffs.html`, wiki pages) instead of a stripped-down one, and the same 260-star golden starfield (`rgba(255,216,120,a)`, no canvas-level opacity dimming) — it used to have a much fainter 120-star white-dot version. The admin panel's ticket detail also got decluttered: the sender's identity/contact/subscription used to print three times (header sub-line, channel badge, account card) — now it's one `.tkt-acc` card with one combined line, and the priority/status/reply controls each got an explicit `.tkt-ctrl-lbl` label; the two rows of filter pills became three plain `<select>` dropdowns. See `AGENTS/architecture/support-system.md` for the full design and what's not implemented (FAQ deflection, CSAT, SLA escalation, a real admin picker for assignment).
