...
- **Payment methods (2026-08-30, ADR-014)**: card (Robokassa), crypto (CryptoPay) and Telegram Stars are ON; ЮMoney is OFF **permanently** — this is a business decision, not a toggle expected to flip back, unlike every earlier round. `bot/utils/settings_store.py` `_DEFAULT_ENABLED` now has `stars: True` (was `False` under ADR-013 for less than a day — that was wrong, Stars was never actually meant to be disabled, and the site was incorrectly telling users it was unavailable; ADR-014 fixed both the default and the copy). A code default only matters for a fresh deploy with no `app_settings` row — an already-toggled prod row wins over it, so if `/admin` → Настройки has Stars/ЮMoney set differently, that wins. Don't remove `yoomoney_payment.py` or the `yoomoney` provider key/toggle even though it's permanently off — same reasoning as before, it's a flip in `/admin`, not a deploy; do NOT reintroduce a "ЮMoney may come back" framing anywhere, this is settled. Because "zero providers enabled" turned out to be a real state at one point, `bot/handlers/profile.py::pay_choice_text()` and `landing/get-vpn.html`'s payment step both explicitly handle it. Crypto payment (via @CryptoBot) always requires Telegram regardless of how the account was created — that's a CryptoPay constraint, not ours; card is the only truly Telegram-independent method. Telegram Stars (XTR) is bot-only by nature — there's no `/api/invoice/stars`, it can't exist as a site checkout option since Stars payments only render inside the Telegram client, not a website form; `landing/wiki/payment.html`'s comparison table reflects this with a `—` in the "На сайте" column rather than "Отключено". Site-based invoice creation (`/api/invoice/card`, `/api/invoice/crypto`, mirrored by `/api/card/plans`/`/api/crypto/plans`) only supports the 3 fixed plan tiers — no custom-day pricing (that only exists in the guest-checkout path, see below).
