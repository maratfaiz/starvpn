...
- **Payment methods (2026-08-30, ADR-010)**: only ЮMoney is active right now — card (Robokassa), crypto (CryptoPay), and Telegram Stars are all toggled off via `app_settings` (`bot/utils/settings_store.py`, `PROVIDER_KEYS` includes `stars` now, not just card/yoomoney/crypto). This is a toggle, not a code deletion — flip it back on from `/admin` → Настройки, no deploy needed. Don't remove `card_payment.py`/`crypto_payment.py`/the Stars invoice code because they "look unused".
