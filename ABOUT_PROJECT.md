...
Вход — email (magic-link, без пароля, без Telegram): POST /api/account/login отправляет письмо, GET /account/verify?token= подтверждает и ставит cookie-сессию (star_session, 30 дней).
