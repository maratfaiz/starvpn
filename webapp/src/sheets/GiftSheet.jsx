    ...
    <BottomSheet open={open} onClose={onClose} maxHeight="78%">
      ...
      <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-2">
            {dayOptions.map((opt) => {
              ...
              return (
                ...
                >
                  <span className="font-display font-bold text-[13.5px]" style={{ color: selected ? "#FFB800" : "#EBE0CC" }}>
                    {opt}
                  </span>
                </button>
              );
        ...
        <button
          ...
          style={{ background: username.trim() ? "linear-gradient(135deg,#FFB800,#D99B00)" : "rgba(255,255,255,.08)" }}
        >
          <StarIcon size={15} color={username.trim() ? "#1A1408" : "#EBE0CC"} />
          <span className="font-display font-bold text-[15px]" style={{ color: username.trim() ? "#1A1408" : "#EBE0CC" }}>
            Подарить {days} дней
          </span>
        </button>
      </div>
